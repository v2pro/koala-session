...
func (encoder *SessionEncoder) Encode(session *recording.Session) {
	if len(session.SessionId) > 0 {
		encoder.Output = append(encoder.Output, []byte{byte(ThriftUTF7), 0x00, 0x01}...)
		encoder.encodeString(session.SessionId)
	}
	if session.CallFromInbound != nil {
		encoder.Output = append(encoder.Output, []byte{byte(ThriftStruct), 0x00, 0x02}...)
		encoder.encodeCallFromInbound(session.CallFromInbound)
	}
	if session.ReturnInbound != nil {
		encoder.Output = append(encoder.Output, []byte{byte(ThriftStruct), 0x00, 0x03}...)
		encoder.encodeReturnInbound(session.ReturnInbound)
	}
	if len(session.Actions) > 0 {
		encoder.Output = append(encoder.Output, []byte{byte(ThriftList), 0x00, 0x04}...)
		encoder.Output = append(encoder.Output, byte(ThriftStruct))
...
func (encoder *SessionEncoder) encodeAction(action recording.Action) {
	...
	case *recording.CallFromInbound:
		encoder.Output = append(encoder.Output, []byte{byte(ThriftStruct), 0x00, 0x01}...)
	...
	case *recording.AppendFile:
		encoder.Output = append(encoder.Output, []byte{byte(ThriftStruct), 0x00, 0x04}...)
		encoder.encodeAppendFile(typedAction)
	case *recording.SendUDP:
		encoder.Output = append(encoder.Output, []byte{byte(ThriftStruct), 0x00, 0x05}...)
...
func (encoder *SessionEncoder) encodeSendUDP(sendUDP *recording.SendUDP) {
	...
	if len(sendUDP.Content) > 0 {
		encoder.Output = append(encoder.Output, []byte{byte(ThriftUTF7), 0x00, 0x03}...)
...
func (encoder *SessionEncoder) encodeAppendFile(appendFile *recording.AppendFile) {
	...
	if len(appendFile.FileName) > 0 {
		encoder.Output = append(encoder.Output, []byte{byte(ThriftUTF7), 0x00, 0x02}...)
...
func (encoder *SessionEncoder) encodeBaseAction(baseAction *recording.BaseAction) {
	encoder.Output = append(encoder.Output, []byte{byte(ThriftI64), 0x00, 0x01}...)
